...
For the final project, I am using digital tools like data visualization to examine, compare, and contrast expenditure pay in different regions of the United States during the 1930s.
...
data <- recreational.data %>%
  group_by(state) %>%
  summarize(total_population = sum(population), total_expenditures = sum(total_expenditures))
data_long <- tidyr::pivot_longer(data, c(total_population, total_expenditures), names_to = "variable", values_to = "value")
ggplot(data_long, aes(x = state, y = value, color = variable)) +
  geom_line() +
