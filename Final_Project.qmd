...
# Filter the data for South Carolina and Tennessee and combine them
rec.data <- recreational.data %>%
  filter(state %in% c("SC", "TN")) %>%
  group_by(state, Year) %>%  # replace 'Year' with your actual column name
  summarize(state.expenditures = sum(total_expenditures))

# Plot the data
ggplot(rec.data, aes(x = Year, y = state.expenditures, color = state, group = state)) +
  geom_line() +
  ggtitle("Expenditure for South Carolina and Tennessee Over Time")
```
