...
```{r}
#7 Creating a line graph for total expenditures in New England during the 1930s (RI and PA)
...
ggplot(rec.data, aes(x = year, y = state.expenditures, color = state, group = state)) +
  geom_line() +
  ggtitle("Total Expenditures for New England Over Time in the 1930s")
```
...
My analysis on the difference in expenditures for New England and Southern states during the 1930s as well as their impact on recreational workers demonstrates that as time went on, the number of workers gradually increased and the economy improved. Additionally, the data suggests that Southern states, with the exception of Texas, paid lower expenditures than the North.

The reason why Texas stood out in terms of expenditure was because during the Great Depression, according to the Texas State Historical Association, the state was the epicenter of the Civilian Conservation Corps programs.[^6] In the article "Great Depression" by Ben Procter and Brian Cervantez, "The CCC, a nationwide program, for young men that focused on natural resources from 1933 to 1942, was very active in Texas. At its peak in 1935 the corps had twenty seven camps in Texas constructing national parks
